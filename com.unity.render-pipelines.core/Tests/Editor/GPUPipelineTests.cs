            ...
            in ClusterCullingResultsCPUReadbackData cullingResults)
        {
            ...
            using (var geoPoolResults = new GeometryPoolCPUReadbackData())
            {
                ...
                using (var bigBufferResults = new BigBufferCPUReadbackData())
                {
                    ...
                    Assert.IsTrue(cullingResults.drawArgs.indexCount > 0, "Should have more than at least 1 index.");
                    ...
                    for (int i = 0; i < expectedVisibleInstances.Length; ++i)
                    {
                        ...
                        int sampledGeometryHandle = (int)bigBufferResults.LoadData<Vector4>(visibleInstanceID.index, GPUInstanceDataBuffer.DefaultSchema._DeferredMaterialInstanceData).x;
